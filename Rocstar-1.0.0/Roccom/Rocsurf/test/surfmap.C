 ...
 *********************************************************************/
// $Id: surfmap.C,v 1.6 2008/12/06 08:43:24 mtcampbe Exp $
...
int main(int argc, char *argv[]) {
  ...
  COM_new_attribute((wname+".normals").c_str(), 'e', COM_DOUBLE, 3, "");
  COM_resize_array((wname+".normals").c_str());
  COM_window_init_done( wname.c_str());

  std::cout << "loading Rocout" << endl;
  COM_LOAD_MODULE_STATIC_DYNAMIC(Rocout, "OUT");

  std::cout << "Computing pane connectivity... " << endl;
  ...
  std::cout << "Computing face normals... " << endl;
  COM_LOAD_MODULE_STATIC_DYNAMIC( Rocsurf, "SURF");

  int SURF_normal = COM_get_function_handle( "SURF.compute_element_normals");
  int nrmls_hdl = COM_get_attribute_handle( (wname+".normals").c_str());

  COM_call_function( SURF_normal, &nrmls_hdl);
