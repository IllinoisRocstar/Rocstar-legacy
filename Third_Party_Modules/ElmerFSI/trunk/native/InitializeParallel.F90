...
#include "../config.h"
       ...
       CALL Info ('ElmerCSCParallel:Initialize', ' Setting FirstTime to FALSE')
       FirstTime = .FALSE.
     END IF
     ...
     IF( .NOT. GotModelName ) THEN
       OPEN( 1, File='./Elmer/Modin/ELMERSOLVER_STARTINFO', STATUS='OLD', ERR=10 )
       READ(1,'(a)') ModelName
       CLOSE(1)
     END IF
...
!     DO WHILE( .TRUE. )
       ...
       IF( MyVerbosity > 3) write (*,*) ' ElmerCSCParallel:InitializeParallel: FirstLoad = ', FirstLoad
...
!------------------------------------------------------------------------------
       ...
       CALL Info('ElmerCSCParallel:ElmerInitialize',' Type of Simulation = '//eq)
