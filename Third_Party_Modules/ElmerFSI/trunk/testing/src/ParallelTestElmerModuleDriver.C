...
namespace ElmerModuleDriver{
  ...
  int ParallelTest(int argc,char *argv[])
  {
    ...
    ElmerModuleDriver::CommType communicator(&argc,&argv);
    ...
    if(!comline.GetOption("help").empty()){
      ...
      if(do_stdout){
        ...
        if(verblevel > 1)
          std::cout << "ElmerModuleDriver::ParallelTest: Exiting test function (success)" 
                    << std::endl;
      }
    ...
    if(clerr){
      if(do_stdout){
        ...
        if(verblevel > 2)
          std::cout << "ElmerModuleDriver::ParallelTest: Exiting test function (fail)" << std::endl;
      }
      communicator.SetExit(1);
    }
    if(communicator.Check())
    ...
    if(!OutFileName.empty()){
      if(Out){
        ...
        if(!Ouf){
          std::cout << "ElmerModuleDriver::ParallelTest> Error: Could not open output file, " 
    ...
    if(verblevel > 1 && Out)
      *Out << "ElmerModuleDriver::ParallelTest: Entering test function" << std::endl;
    
    // Make an instance of the ElmerModuleDriver testing object, ElmerModuleDriver::ParallelTestingObject
    ...
    // run the listed tests.
      ...
      if(!ListInf){
        if(Out)
          *Out << "ElmerModuleDriver::ParallelTest> Error: Could not open list of tests in file " 
    ...
    if((verblevel > 1) && Out)
      *Out << "ElmerModuleDriver::ParallelTest: Exiting test function (success)" << std::endl;
...
{
  return(ElmerModuleDriver::ParallelTest(argc,argv));
}
